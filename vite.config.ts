...
[신념/가정 갱신]
- 새 텍스트가 기존 신념 중 하나와 본질적으로 같은 믿음을 다시 보여준다면: 그 신념을 새로 추가하지 말고, 같은 문장을 유지한 채 confidence를 5~15 사이로 올리고 evidenceCount를 1 늘리세요. 이때 quote에는 이번 새 텍스트에서 그 신념을 뒷받침하는 실제 구절(직접 인용하거나 짧게 다듬어서)을 넣으세요.
- 기존 목록에 없는 진짜 새로운 신념이 드러난다면: confidence 40~60, evidenceCount 1로 새 항목을 추가하고, quote에 마찬가지로 새 텍스트에서 근거가 된 구절을 넣으세요.
- 이번 텍스트와 무관해서 그대로 유지되는 기존 신념은 quote를 null로 두세요 (근거가 오래된 것처럼 지어내지 마세요).
...
{
  "beliefs": [ { "domain": "한 단어 영역, 예: 커리어/관계/돈/일/자기인식", "statement": "신념 자체를 말하는 문장 (있었던 일의 요약이 아닐 것)", "confidence": 정수 0-100, "evidenceCount": 정수, "quote": "string 또는 null" } ],

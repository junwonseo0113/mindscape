...
export function sanitizeStore(parsed: any): Store {
  ...
  return {
      ...
      literaryMatch: parsed.literaryMatch && typeof parsed.literaryMatch === "object"
          && typeof parsed.literaryMatch.passageId === "string"
        ? {
            passageId: parsed.literaryMatch.passageId,
            themes: Array.isArray(parsed.literaryMatch.themes) ? parsed.literaryMatch.themes.filter((t: any) => typeof t === "string") : [],
            selectedDate: typeof parsed.literaryMatch.selectedDate === "string" ? parsed.literaryMatch.selectedDate : "",
          }
        : null,
      literaryMatchSnapshot: typeof parsed.literaryMatchSnapshot === "number" ? parsed.literaryMatchSnapshot : undefined,
      // Forward-compatible field (see StoredLensSession in types.ts) — a
      // store saved before this field existed simply has none yet.
      lensSessions: Array.isArray(parsed.lensSessions)
        ? parsed.lensSessions.filter((s: any) => s && typeof s.id === "string" && typeof s.kind === "string" && typeof s.summary === "string")
        : [],
  };
}
...
// One shared log for every structured-thinking lens (see StoredLensSession
// in types.ts) — forward-compatible infrastructure only. Nothing in the
// current UI calls this yet; it exists so a future longitudinal-
// intelligence pass has a ready-made, already-sanitized place to write
// into instead of needing its own data-model change at that point.
export function appendLensSession(
  prev: Store,
  kind: LensKind,
  summary: string,
  linked?: { entryId?: string; beliefId?: string }
): Store {
  const session: StoredLensSession = {
    id: `lens-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
    kind,
    date: formatDateDots(new Date()),
    linkedEntryId: linked?.entryId,
    linkedBeliefId: linked?.beliefId,
    summary,
  };
  return {
    ...prev,
    lensSessions: [...prev.lensSessions, session].slice(-100),
  };
}
...
export function mergeAnalysisIntoStore(prev: Store, result: any, rawText: string, sessionSummary?: string, accumulate: boolean = true): Store {
  ...
  // Forward-compatible only (see StoredHistoryEntry.actDefusion in
  // types.ts) — derived purely from the two fields above (already part of
  // the existing analyze schema), never read by any screen yet.
  const actDefusion = thoughtLabelSuggestion && distancedReframeSuggestion
    ? { thoughtLabel: thoughtLabelSuggestion, reframe: distancedReframeSuggestion }
    : null;
  const historyEntry: StoredHistoryEntry = { id: entryId, date: today, text: rawText, analysis, languageObservation, sessionSummary, actDefusion };

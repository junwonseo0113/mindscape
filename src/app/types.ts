...
export type StoredHistoryEntry = {
  ...
  // (see src/server/aiHandlers.ts's handleSummarizeSession). Optional
  // because it requires its own API round trip that can fail independently
  // of the main analysis; stored here so past-session previews can reuse it.
  sessionSummary?: string;
  // Forward-compatible data slots for a future structured-thinking-lens
  // pass (ACT defusion / CFT self-compassion) — not populated by the
  // current analysis pipeline and not read by any screen yet. Reserved
  // here only so that pipeline can start writing to a stable shape without
  // another types.ts change later; see StoredLensSession below for the
  // session-log side of the same future work.
  actDefusion?: { thoughtLabel: string; reframe: string } | null;
  selfCompassionNote?: string | null;
};

// The structured-thinking lenses a future pass may offer alongside the
// app's own default analysis (CBT/ACT/CFT/MBCT and beyond) — not exposed
// as a product surface anywhere yet (no lens picker, no per-framework UI).
// Defined now, together with StoredLensSession below, purely as forward-
// compatible data infrastructure: a place for a later longitudinal-
// intelligence pass (see Store.lensSessions) to write into without a
// schema change at that point.
export type LensKind = "cbt" | "act" | "cft" | "mbct" | "sixHats" | "firstPrinciples" | "schema" | "ba";

// One append-only log shared by every lens, instead of each lens keeping
// its own separate history — this is what would let a future pass reason
// about them as one connected toolkit rather than several unrelated
// features (e.g. "this same belief has come up in a lens session 3 times
// before"). Not written to or read by any screen currently.
export type StoredLensSession = {
  id: string;
  kind: LensKind;
  date: string;
  linkedEntryId?: string;
  linkedBeliefId?: string;
  // One sentence on what this session actually produced (the reframe
  // reached, the conclusion drawn) — not a transcript of every step.
  summary: string;
};
...
// Premium's "For where you are now" literary match — the passage itself
// always comes from the curated LITERARY_PASSAGES library (see
// literaryPassages.ts), never generated text; this just records WHICH
// passage was selected, why (the themes that won), and against how much
// data, so the match persists across visits instead of re-selecting every
// time Premium opens (see literaryPassages.ts's selectLiteraryPassage and
// App.tsx's LiteraryReflectionPanel).
export type StoredLiteraryMatch = {
  passageId: string;
  themes: string[];
  selectedDate: string;
};
...
export type Store = {
  ...
  // null/undefined = not enough data yet, or never computed. See
  // StoredLiteraryMatch and literaryPassages.ts.
  literaryMatch?: StoredLiteraryMatch | null;
  // beliefs.length + history.length at the moment literaryMatch was last
  // computed — same "only reconsider once there's meaningfully new data"
  // gate goalsBeliefSnapshot uses, so opening Premium repeatedly never
  // reshuffles the passage on its own.
  literaryMatchSnapshot?: number;
  // Every completed structured-thinking-lens session, regardless of which
  // lens or what it was linked to — see StoredLensSession above. Forward-
  // compatible longitudinal-intelligence infrastructure only: nothing
  // currently writes to this array, and no screen reads it.
  lensSessions: StoredLensSession[];
};

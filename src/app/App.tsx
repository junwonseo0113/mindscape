...
// A single quick streak across the upper sky — the trail is baked into the
// element itself (a short gradient bar, bright head fading to nothing at
// the tail) and the whole thing just translates along its own angle, so it
// reads as one star falling rather than a looping animation. Purely
// decorative: pointer-events none, never touches layout.
function ShootingStar({ top, left, angle, length }: { top: number; left: number; angle: number; length: number }) {
  const rad = (angle * Math.PI) / 180;
  const travel = length * 1.8;
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 1.1, ease: "easeIn", times: [0, 0.12, 0.7, 1] }}
      style={{ position: "absolute", top: `${top}%`, left: `${left}%`, pointerEvents: "none", zIndex: 1 }}
    >
      <div
        style={{
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
        ...
        <motion.button onClick={onClose} whileTap={{ opacity: 0.6 }} style={{ border: 0, background: "transparent", color: vtgCreamMuted, cursor: "pointer", fontSize: 16 }}>×</motion.button>
      </div>
...
function ScreenHome({ onNavSelect, onStartThink, onOpenBrainMap, store }: { onNavSelect?: (id: string) => void; onStartThink?: () => void; onOpenBrainMap?: (region?: CognitiveRegion) => void; store: Store }) {
  ...
  // An occasional shooting star in the sky — fires once after a random
  // 3-8s delay, then schedules the next one 9-23s later, so it never falls
  // on a predictable rhythm. Respects prefers-reduced-motion (skipped
  // entirely rather than just made instant, since it's pure decoration).
  const [star, setStar] = React.useState<{ id: number; top: number; left: number; angle: number; length: number } | null>(null);
  ...
  React.useEffect(() => {
    ...
    const fire = () => {
      ...
      setStar({
        ...
        angle: 115 + Math.random() * 30,
        length: 70 + Math.random() * 40,
      });
      clearId = setTimeout(() => setStar((s) => (s?.id === id ? null : s)), 1200);
      timeoutId = setTimeout(fire, 9000 + Math.random() * 14000);
    };
    timeoutId = setTimeout(fire, 3000 + Math.random() * 5000);
    return () => { clearTimeout(timeoutId); clearTimeout(clearId); };
  }, [reduceMotion]);
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {star && <ShootingStar key={star.id} top={star.top} left={star.left} angle={star.angle} length={star.length} />}
      </AnimatePresence>
...
function GoalDirectionsPanel({ store, updateStore }: { store: Store; updateStore: (updater: (prev: Store) => Store) => void }) {
  ...
  return (
    <div style={{ marginBottom: 14, backgroundColor: mdCard, borderRadius: 20, padding: "16px 18px", boxShadow: mdCardShadow }}>
      <div style={{ ...sans, fontSize: 11, fontWeight: 800, letterSpacing: "0.08em", color: mdAccentText }}>WHERE YOU MIGHT BE HEADED</div>

// The actual business logic behind every /api/* endpoint — prompts,
// validation, response shaping — lives here instead of in either entry
// point that calls it. There are two entry points on purpose: vite.config.ts's
// dev-server middleware (so `npm run dev` keeps working exactly as before)
// and the Vercel serverless functions under /api (the only thing that
// actually runs once this is deployed — Vite's own dev-server middleware
// hook does not exist in a production `vite build`, so without this split
// the entire "Speak your mind" analysis loop would silently do nothing in
// production). Both entry points are thin adapters over the functions
// below; the prompts and rules only exist in one place, unchanged from what
// vite.config.ts used to run inline (same model, same prompts, same JSON
// schemas — this is purely a relocation for production reachability, not a
// behavior change).

export type HandlerResult = { status: number; json: any };

// Extracts the "===JSON===" marker convention shared by every endpoint —
...
export function extractJsonAfterMarker(rawText: string): any | null {
...
async function callClaude(apiKey: string, prompt: string, maxTokens: number): Promise<HandlerResult | { text: string }> {
  ...
  if (!apiRes.ok) {
    ...
    return { status: 502, json: { error: `The AI call failed (${apiRes.status}): ${errText.slice(0, 300)}` } }
  }
...
function isHandlerResult(v: HandlerResult | { text: string }): v is HandlerResult {
  return 'status' in v
}

// Powers the "disagree -> give me a genuinely different reading of the
// same evidence" loop: takes the interpretation the user just rejected
// plus everything already rejected before it, and either returns a real
// alternative or explicitly says there isn't one (exhausted) instead of
// inventing a cosmetic rewording.
export async function handleReinterpret(apiKey: string, body: any): Promise<HandlerResult> {
  const { currentText, evidenceQuotes, rejectedTexts } = body ?? {}
  if (!currentText || typeof currentText !== 'string' || !currentText.trim()) {
    return { status: 400, json: { error: 'There is nothing to reinterpret.' } }
  }
...

{
  ...
  if (isHandlerResult(result)) return result

  const parsed = extractJsonAfterMarker(result.text)
  if (!parsed) return { status: 502, json: { error: "Couldn't parse the AI response." } }
  return { status: 200, json: parsed }
}

export async function handleAnalyze(apiKey: string, body: any): Promise<HandlerResult> {
  const { text, matchableBeliefs, matchablePending, priorAssumptions, priorConnections, aspiration, name } = body ?? {}
  if (!text || typeof text !== 'string' || !text.trim()) {
    return { status: 400, json: { error: 'There is no text to analyze.' } }
  }
...
"""`
  ...
  const result = await callClaude(apiKey, prompt, 3072)
  if (isHandlerResult(result)) return result
...
// Feature 3 — a separate, deliberately narrow call from handleAnalyze:
// pure restatement of what was said, no interpretation/labels (those stay
// in handleAnalyze's own output). Called in parallel with handleAnalyze
// from ScreenProcessing; the caller treats a failure here as "no summary
// this time," never as a reason to fail the whole session.
export async function handleSummarizeSession(apiKey: string, body: any): Promise<HandlerResult> {
  const { text } = body ?? {}
  if (!text || typeof text !== 'string' || !text.trim()) {
    return { status: 400, json: { error: 'There is no text to summarize.' } }
  }
...

{
  ...
  const result = await callClaude(apiKey, prompt, 512)
  if (isHandlerResult(result)) return result

  const parsed = extractJsonAfterMarker(result.text)
  if (!parsed || typeof parsed.summary !== 'string') return { status: 502, json: { error: "Couldn't parse the AI response." } }
  return { status: 200, json: { summary: parsed.summary } }
}

// Feature 4 — the Home screen's "Where you might be headed" widget. Takes
// the recurring unconscious beliefs the Brain Map has already surfaced
// (domain + statement + evidence count only — no raw entry text, same
// minimal-payload spirit as handleReinterpret) and infers a few
// forward-looking growth directions. Called only when the belief set has
// changed since the last computation (see App.tsx's HomeGoalsWidget) —
// not on every Home visit.
export async function handleInferGoals(apiKey: string, body: any): Promise<HandlerResult> {
  const { beliefs } = body ?? {}
  const list: { domain: string; statement: string; evidenceCount: number }[] = Array.isArray(beliefs)
    ...
    : []
  if (list.length === 0) {
    return { status: 400, json: { error: 'No beliefs to work from.' } }
  }
...

{
  ...
  const goals = Array.isArray(parsed?.goals)
    ...
    : null
  if (!goals || goals.length === 0) return { status: 502, json: { error: "Couldn't parse the AI response." } }
  return { status: 200, json: { goals } }
}
